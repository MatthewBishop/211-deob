public class ClientPacket implements class278 {
	...
	static int decodeMask(ExampleBuffer var1) {
		int mask = var1.readUnsignedByte();
		if ((mask & 2) != 0) {
			int readByte = var1.readUnsignedByte();
			mask += readByte << 8;
		}
	...
	static class ExampleBuffer {
		
		public byte[] array;

		public int offset;
		
		public ExampleBuffer(byte[] array) {
			this.array = array;
			this.offset = 0;
		}
	...
	static int decodeMask(PacketBuffer var1) {
		int mask = var1.readUnsignedByte();
		if ((mask & 2) != 0) {
			int readByte = var1.readUnsignedByte();
			mask += readByte << 8;
	...
	static final void updateNpcs(boolean var0, PacketBuffer var1) {
		...
		int var23;
		while (true) {
			var2 = ServerPacket.field2674 ? 16 : 15;//npc index good
			var3 = 1 << var2;
			...
			if (ServerPacket.field2674) {
				...
				var11 = var1.readBits(1);//mask needed?
				if (var11 == 1) {
					Client.field426[++Client.field425 - 1] = var4;
				}

				boolean var12 = var1.readBits(1) == 1;//unused? I think. Steam client only
				if (var12) {
					var1.readBits(32);
				}

				var7 = var1.readBits(1);//teleported?
				if (var0) {
					var9 = var1.readBits(8);//y valid
					if (var9 > 127) {
						var9 -= 256;
					}
				} else {
				...
				var20.definition = class137.getNpcDefinition(var1.readBits(14));//read npc id
			} else {
				if (var0) {
		...
		for (var2 = 0; var2 < Client.field425; ++var2) {
			...
			int var21 = var1.readUnsignedByte();//mask1
			if ((var21 & 2) != 0) {//mask2 if mask1 big
				int var6 = var1.readUnsignedByte();
				var21 += var6 << 8;
			}

			if ((var21 & 16384) != 0) {//mask3 if mask is super big.
				int var6 = var1.readUnsignedByte();
				var21 += var6 << 16;
			}

//			int var21 = var1.readMedium();

			System.err.println("Parsing mask " + var21 + " for " + var14.definition.id);
