...
public final class Client extends GameEngine implements Usernamed, OAuthApi {
	...
	//public static final String IP = "35.229.28.49";
			...
			if (--js5Cycles + 1 <= 0) {
				try {
					if (js5ConnectState == 0) {
						WorldMapManager.js5SocketTask = GameEngine.taskHandler.newSocketTask(IP,
								43594);
						++js5ConnectState;
					}
		...
		try {
			...
			if (loginState == 1) {
				if (BufferedSource.socketTask == null) {
					BufferedSource.socketTask = GameEngine.taskHandler.newSocketTask(IP,
							43594);
				}
